...
/* Text Summarizer by hxii
https://0xff.nu/text-summarization
 */
...
function score_sentence(string $sentence, array $keywords) {
	...
	foreach ($keywords as $keyword => $frequency) {
		$offset = 0;
		$positions = [];
		while (($pos = stripos($sentence, $keyword, $offset)) !== FALSE) {
			/* find all positions of keyword in sentence */
...
function merge_similar(array $keywords) {
	foreach ($keywords as $i => $k) {
		foreach ($keywords as $j => $k2) {
			...
			if ($j > $i && (max($perc, $perc2) >= 85)) {
				/* if the keywords are at least 85% similar, merge them */
